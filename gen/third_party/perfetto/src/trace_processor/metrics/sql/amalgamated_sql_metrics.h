...
R"_d3l1m1t3r_(       'slice' AS track_type
...
R"_d3l1m1t3r_(       WHEN 0 THEN 'None'
       WHEN 1 THEN 'AC'
       WHEN 2 THEN 'USB'
...
R"_d3l1m1t3r_(      'sleep_screen_doze_ns',
...
R"_d3l1m1t3r_(        'blocking_method', blocking_method,
...
R"_d3l1m1t3r_(    WHERE s.startup_id = $startup_id AND s.slice_name GLOB $slice_name
...
R"_d3l1m1t3r_(        'ts_method_start', s.slice_ts
        ))
...
R"_d3l1m1t3r_(      WHERE p.startup_id =launches.startup_id
    ),
...
R"_d3l1m1t3r_(          dur_of_process_running_concurrent_to_launch(launches.startup_id, '*installd') > 0
...
R"_d3l1m1t3r_(CREATE VIEW android_startup_event AS
SELECT
  'slice' AS track_type,
...
R"_d3l1m1t3r_(CREATE PERFETTO TABLE android_jank_cuj_sf_process AS
SELECT * FROM process
...
R"_d3l1m1t3r_(CREATE PERFETTO TABLE android_jank_cuj_sf_render_engine_thread AS
...
R"_d3l1m1t3r_(  launches.package AS name,
  launches.startup_id,
  ROW_NUMBER() OVER(PARTITION BY launches.startup_id ORDER BY functions.ts ASC) AS number
FROM functions
JOIN android_startups launches ON launches.package GLOB '*' || functions.process_name || '*'
...
R"_d3l1m1t3r_(  SELECT slice_name, startup_id, is_main_thread
  FROM android_thread_slices_for_all_startups s
...
R"_d3l1m1t3r_(  'BINDER_TRANSACTION_REPLY_SLICES_FOR_LAUNCH(startup_id INT, threshold DOUBLE)',
  'name STRING',
  '
...
R"_d3l1m1t3r_(  'slice_table_name', '{{slice_table_name}}',
...
const char kChromeChromeScrollJankV2[] = R"_d3l1m1t3r_(--
-- Copyright 2020 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
R"_d3l1m1t3r_(DROP VIEW IF EXISTS chrome_presented_gesture_scrolls;
...
R"_d3l1m1t3r_(  MIN(start_ts) AS start_ts,
...
R"_d3l1m1t3r_(  descendant_slice_end(slice.id, "LatchToSwapEnd") AS input_latency_end_ts,
...
R"_d3l1m1t3r_(FROM chrome_presented_gesture_scrolls frames
...
R"_d3l1m1t3r_(  ON jank_cause.slice_id = frames.event_latency_id;
...
R"_d3l1m1t3r_(-- @column max_start_ts            OS timestamp of the last touch move arrival within a frame.
...
R"_d3l1m1t3r_(-- @column delay_since_last_input difference in OS timestamps of inputs in the current and the previous frame.
...
R"_d3l1m1t3r_(-- @column cause_of_jank          The reason the Vsync was missed.
...
R"_d3l1m1t3r_(GROUP BY scroll_id;
...
R"_d3l1m1t3r_(DROP VIEW IF EXISTS chrome_scroll_jank_v3_output;
...
const char kChromeEventLatencyScrollJank[] = R"_d3l1m1t3r_(--
-- Copyright 2022 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
R"_d3l1m1t3r_(-- because coalesced ones are not shown on the screen.
...
R"_d3l1m1t3r_(  ON filtered_scroll_event_latency.ts >= scroll_event_latency_begins.ts
...
R"_d3l1m1t3r_(CREATE VIEW scroll_event_latency_updates_with_ends
AS
SELECT
...
R"_d3l1m1t3r_(  LAG(id) OVER sorted_events AS prev_id,
...
R"_d3l1m1t3r_(DROP VIEW IF EXISTS scroll_event_latency_jank;
...
const char kChromeEventLatencyScrollJankCause[] = R"_d3l1m1t3r_(--
-- Copyright 2022 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
R"_d3l1m1t3r_(ON cur_breakdowns.next_event_latency_id = next_breakdowns.event_latency_id AND
...
R"_d3l1m1t3r_(ON cur_breakdowns.prev_event_latency_id = prev_breakdowns.event_latency_id AND
...
R"_d3l1m1t3r_(-- and |event_latency_scroll_breakdowns_prev_jank_deltas| tables and select the maximum |delta_dur_ns| of them.
...
R"_d3l1m1t3r_(  COALESCE(prev.prev_jank, 0) as prev_jank,
...
R"_d3l1m1t3r_(  event_latency_id as slice_id,
  track_id,
...
R"_d3l1m1t3r_(  ON cur_event_latency.max_jank_breakdown_id = cur_sub_breakdowns.parent_id
...
const char kChromeEventLatencyToBreakdowns[] = R"_d3l1m1t3r_(--
-- Copyright 2022 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
R"_d3l1m1t3r_(DROP VIEW IF EXISTS event_latency_breakdowns;
...
R"_d3l1m1t3r_(      "RendererMainProcessing", "EndActivateToSubmitCompositorFrame",
...
R"_d3l1m1t3r_(      "RendererCompositorFinishedToSendBeginMainFrame",
...
R"_d3l1m1t3r_(  max(CASE WHEN name = "GenerationToRendererCompositor" THEN dur END) AS GenerationToRendererCompositorNs,
...
R"_d3l1m1t3r_(  max(CASE WHEN name = "RendererMainStartedToTermination" THEN dur END) AS RendererMainStartedToTerminationNs,
...
R"_d3l1m1t3r_(  max(CASE WHEN name = "RendererCompositorFinishedToSubmitCompositorFrame" THEN dur END) AS RendererCompositorFinishedToSubmitCompositorFrameNs,
...
R"_d3l1m1t3r_(  max(CASE WHEN name = "SubmitCompositorFrameToPresentationCompositorFrame" THEN dur END) AS SubmitCompositorFrameToPresentationCompositorFrameNs,
...
const char kChromeJankUtilities[] = R"_d3l1m1t3r_(--
-- Copyright 2022 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
R"_d3l1m1t3r_(-- floating-point numbers. Value 1e-9 have been chosen as follows: the ratio has
-- nanoseconds in numerator and VSync interval in denominator. Assuming refresh
-- rate more than 1 FPS (and therefore VSync interval less than a second), this
-- ratio should increase with increments more than minimal value in numerator
-- (1ns) divided by maximum value in denominator, giving 1e-9.
...
R"_d3l1m1t3r_(-- Returns true if the frame was janky, false otherwise
RETURNS BOOL AS
SELECT
    CASE WHEN
...
R"_d3l1m1t3r_(-- null check for cases when the neighbor frame count can be null for the
...
R"_d3l1m1t3r_(  WHEN $name GLOB "*HomeButton*" THEN
    "HomeButton"
...
const char kChromeVsyncIntervals[] = R"_d3l1m1t3r_(-- A simple table that checks the time between VSync (this can be used to
-- determine if we're refreshing at 90 FPS or 60 FPS).
--
-- Note: In traces without the "Java" category there will be no VSync
--       TraceEvents and this table will be empty.
...
R"_d3l1m1t3r_(  AND NOT EXISTS (SELECT * FROM DisplayCompositorPresentationEvents)
GROUP BY ts;
...
const FileToSql kFileToSql[] = {
  ...
  {"chrome/chrome_scroll_jank_v2.sql", kChromeChromeScrollJankV2},
  ...
  {"chrome/event_latency_scroll_jank.sql", kChromeEventLatencyScrollJank},

  {"chrome/event_latency_scroll_jank_cause.sql", kChromeEventLatencyScrollJankCause},

  {"chrome/event_latency_to_breakdowns.sql", kChromeEventLatencyToBreakdowns},
  ...
  {"chrome/jank_utilities.sql", kChromeJankUtilities},
